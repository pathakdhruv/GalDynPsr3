...
def shk(dkpc, sigd):
  ...
  mu_alphar, sigmuar = input("Enter the proper motion of the pulsar in right ascension (mas yr^-1), error in mu_alpha(sigmua) [separated by comma]: ").split((','))
  mu_deltar, sigmudr = input("Enter the proper motion of the pulsar in declination (mas yr^-1), error in mu_delta(sigmud) [separated by comma]: ").split((','))
